...
# Add this line to ensure all transforms and parameters are properly initialized
...
print("  Initial force balance solved.")
...
# Create an OmnigenousField instance with the desired helicity
omni_field = OmnigenousField(L_B=eq.L, M_B=eq.M, NFP=eq.NFP, helicity=(1, 0))
print(f"  OmnigenousField created with helicity: {omni_field.helicity}")

obj_omnigenity = Omnigenity(
    eq=eq,
    field=omni_field, # Pass the OmnigenousField instance here
    weight=1e1,
    normalize=True
)
obj_omnigenity.build(eq)
print(f"  Omnigenity objective created. Initial omnigenity score: {obj_omnigenity.compute():.4e}")
...
obj_mercier.build(eq)
print(f"  Mercier Stability objective created. Initial Mercier score: {obj_mercier.compute():.4e}")
...
obj_force.build(eq)
print(f"  Force Balance objective created. Initial force balance score: {obj_force.compute():.4e}")
...
print("  Constraints defined: FixPressure, FixIota, FixPsi, FixBoundaryR, FixBoundaryZ.")
...
objective.build(eq)
print(f"  Combined objective function created. Initial total objective value: {objective.compute():.4e}")
...
print("  SIMSOPT surface created from optimized DESC equilibrium.")
print(f"  SIMSOPT surface mpol={s_simsopt.mpol}, ntor={s_simsopt.ntor}")
...
print(f"  {len(coils_all)} coils initialized.")
print(f"  Initial current for each coil: {base_current:.1e} A.")
...
print(f"  SquaredFlux objective created. Initial value: {J_flux.J():.4e}")

# B. Length Penalty (Cost/Inductance)
J_length = QuadraticPenalty(0.0)
for coil in coils:
    J_length = J_length + CurveLength(coil.curve)
print(f"  Length Penalty objective created. Initial value: {J_length.J():.4e}")
...
print(f"  Curvature Penalty objective created. Initial value: {J_curvature.J():.4e}")
...
print(f"  Coil-to-Coil Distance objective created. Initial value: {J_cc_dist.J():.4e}")
...
print(f"  Coil-to-Plasma Distance objective created. Initial value: {J_cs_dist.J():.4e}")
...
print(f"  Initial Total SIMSOPT Objective Value: {JF.J():.4e}")
